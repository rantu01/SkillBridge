...
import { Star, ArrowRight, Download, Calendar, DollarSign, Award, Loader2, Wallet, ArrowDownRight, ArrowUpRight } from 'lucide-react';
            ...
            if (currentUser) {
                try {
                    ...
                    if (data.success && data.user) {
                        setUser(data.user);
                        const bookingsRes = await fetch(`/api/booking?userID=${currentUser.uid}&providerID=${currentUser.uid}`);
                        const bookingsData = await bookingsRes.json();
                        if (bookingsData.success) {
                            const activity = (bookingsData.bookings || [])
                                .map((booking) => {
                                    const servicePrice = Number(booking.service?.price || 0);
                                    const createdAt = booking.reviewedAt || booking.updatedAt || booking.createdAt || booking.timeSlot;

                                    if (booking.requesterID === currentUser.uid && ['Approved', 'In Progress', 'Completed'].includes(booking.status)) {
                                        return {
                                            id: booking._id,
                                    ...
                                    if (booking.providerID === currentUser.uid && booking.status === 'Completed') {
                                        return {
                                            id: booking._id,
                                            title: booking.service?.title || 'Service booking',
                                            kind: 'Earning',
        ...
        <div className="space-y-10 pb-10">
            ...
            <div className="bg-white p-10 rounded-[40px] shadow-sm border border-gray-50 uppercase">
                ...
                <p className="text-gray-500 mt-2 text-lg normal-case">
                    You have <span className="text-blue-600 font-bold underline">{user?.credits ?? 0} credits</span> available right now.
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-8 rounded-[35px] shadow-sm border border-gray-50 flex flex-col gap-4 md:col-span-3 lg:col-span-1">
                    <div className="flex items-center gap-4">
                        <div className="bg-blue-50 p-4 rounded-2xl"><Wallet className="text-blue-500" /></div>
                        <div>
                            <p className="text-gray-400 text-sm font-bold uppercase">Credit Balance</p>
                            <h3 className="text-3xl font-black text-gray-800">{Number(user?.credits || 0)}</h3>
                        </div>
                    </div>
                    <div className="text-xs font-bold px-3 py-1 rounded-full bg-blue-50 w-fit text-blue-700 border border-blue-100">
            ...
            <div className="bg-white rounded-[40px] p-8 shadow-sm border border-gray-50">
                <div className="flex justify-between items-center mb-8">
                    <h2 className="text-2xl font-black text-gray-900">Credit Transactions</h2>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="text-gray-400 text-xs font-black uppercase tracking-wider border-b border-gray-50">
                                ...
                                <th className="pb-4">Booking Status</th>
                                <th className="pb-4 text-right">Credit Delta</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                            {creditActivity.length > 0 ? creditActivity.map((row, i) => (
                                <tr key={i} className="group hover:bg-gray-50/50 transition-all">
                                    <td className="py-5">
                                        <div className="flex items-center gap-3">
                                            <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-bold ${row.bg} ${row.color}`}>{row.delta > 0 ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}</div>
                                            <div>
                                                <span className="font-bold text-gray-700 block">{row.title}</span>
                                                <span className="text-[10px] text-gray-400 font-bold uppercase">Booking #{row.id.slice(-6)}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-5"><span className="px-3 py-1 bg-gray-100 rounded-lg text-[10px] font-black uppercase text-gray-500">{row.kind}</span></td>
                                    <td className="py-5 text-gray-500 font-medium text-sm">{new Date(row.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric', year: 'numeric' })}</td>
                                    <td className="py-5">
                                    ...
                                    <td className={`py-5 text-right font-black ${row.color}`}>{row.delta > 0 ? '+' : ''}{row.delta}<span className="text-[10px] ml-0.5">Cr</span></td>
                                </tr>
                            )) : (
                                <tr>
